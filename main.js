...
function runPrototypeCheck() {
  ...
  if (!viewModel.runtime.isStandalone) issues.push("aun no se esta probando en modo standalone");
  if (state.recipes.length === 0) issues.push("todavia no hay recetas cargadas");
  if (state.plans.meals.length === 0) issues.push("no hay comidas planificadas");
  if (state.plans.sessions.length === 0) issues.push("no hay sesiones futuras programadas");
  if (Object.keys(state.sleepEntries).length === 0) issues.push("falta sueno real para leer recuperacion");
  if (Object.keys(state.weekly?.checklists || {}).length === 0) issues.push("falta al menos un reset semanal");

  if (issues.length === 0) {
    setStatus("Estado del prototipo: base suficiente para empezar a jugar con uso real.");
    return;
  }

  setStatus(`Estado del prototipo: ${issues.slice(0, 3).join(" · ")}.`);
}

async function loadPrototypeDemo() {
  const demoToday = todayKey();
  const weekKey = currentWeekStartKey();
  const nextState = {
    ...viewModel.state,
    profile: {
      ...viewModel.state.profile,
      displayName: viewModel.state.profile.displayName || "Segundo cerebro demo"
    },
    recipes: viewModel.state.recipes.length
      ...
      : [
          {
            ...
            servings: 2,
            totals: { calories: 980, protein: 62, carbs: 96, fat: 28 },
            canonicalIngredients: ["Arroz", "Pollo", "Cebolla"],
            familyCoverage: ["Cereal/tuberculo", "Ave y huevos", "Verduras"],
            ingredients: [
              { name: "arroz", calories: 360, protein: 7, carbs: 78, fat: 1 },
              { name: "pollo", calories: 330, protein: 55, carbs: 0, fat: 7 },
          ...
          },
          {
            ...
            name: "Tortilla suave",
            servings: 2,
            totals: { calories: 620, protein: 34, carbs: 18, fat: 42 },
            canonicalIngredients: ["Huevo", "Patata"],
            familyCoverage: ["Ave y huevos", "Cereal/tuberculo"],
            ingredients: [
              { name: "huevo", calories: 280, protein: 24, carbs: 2, fat: 20 },
              { name: "patata", calories: 180, protein: 4, carbs: 40, fat: 0 }
            ]
          }
        ],
    plans: {
      meals: viewModel.state.plans.meals.length
        ? viewModel.state.plans.meals
        : [
            {
              id: 2001,
              date: demoToday,
      ...
      sessions: viewModel.state.plans.sessions.length
        ? viewModel.state.plans.sessions
        : [
            {
              id: 3001,
              date: demoToday,
              ...
              status: "planned",
              routineName: "Lower A",
              notes: "Sesion demo para probar cierre rapido"
            }
          ]
    },
    mealPlan: Object.keys(viewModel.state.mealPlan).length
      ? viewModel.state.mealPlan
      : {
          [demoToday]: {
            Comida: { id: 2001, name: "Bowl de arroz con pollo", status: "planned" },
            Cena: { id: 2002, name: "Tortilla suave", status: "planned" }
          }
        },
    training: {
      ...viewModel.state.training,
      routines: viewModel.state.training.routines.length
        ? viewModel.state.training.routines
        : [
            { id: 4001, name: "Lower A", focus: "Gluteo y pierna", exercises: "Hip thrust\nSentadilla\nPrensa" }
          ]
    },
    cycle: {
      ...viewModel.state.cycle,
      periodDays: viewModel.state.cycle.periodDays.length ? viewModel.state.cycle.periodDays : [demoToday],
      symptomLog: Object.keys(viewModel.state.cycle.symptomLog || {}).length
        ? viewModel.state.cycle.symptomLog
        : {
            [demoToday]: [
              {
    ...
    sleepEntries: Object.keys(viewModel.state.sleepEntries).length
      ? viewModel.state.sleepEntries
      : {
          [demoToday]: { date: demoToday, hours: 7.4, quality: 4, sleepStart: "23:45", sleepEnd: "07:10", notes: "" }
        },
    calendar: {
      ...viewModel.state.calendar,
      events: viewModel.state.calendar.events.length
        ? viewModel.state.calendar.events
        : [{ id: 6001, title: "Compra semanal", category: "Personal", date: demoToday, time: "18:30", note: "" }]
    },
    schedule: {
      ...viewModel.state.schedule,
      blocks: viewModel.state.schedule.blocks.length
        ? viewModel.state.schedule.blocks
        : [{ id: 7001, title: "Reset semanal", day: "Domingo", start: "19:00", end: "20:00", category: "Planificacion" }]
    },
    weekly: {
      ...viewModel.state.weekly,
      checklists: Object.keys(viewModel.state.weekly?.checklists || {}).length
        ? viewModel.state.weekly.checklists
        : {
            [weekKey]: [
              { id: 8001, title: "Cerrar compra semanal", done: false },
              { id: 8002, title: "Dejar 2 comidas faciles listas", done: false }
            ]
          },
      reviews: viewModel.state.weekly?.reviews || {}
    },
    notes: {
      ...viewModel.state.notes,
      "qa-prototipo": viewModel.state.notes["qa-prototipo"] || "Abrir Home, cerrar previsto vs real, aplicar reset, exportar backup."
    }
  };

  await persistState(nextState, "Datos demo cargados para probar la V1.");
}
...
function wireUi() {
  ...
  const prototypeCheckButton = document.getElementById("prototype-check-button");
  if (prototypeCheckButton) {
    prototypeCheckButton.addEventListener("click", runPrototypeCheck);
  }

  const loadDemoButton = document.getElementById("load-demo-button");
  if (loadDemoButton) {
    loadDemoButton.addEventListener("click", async () => {
      await loadPrototypeDemo();
    });
  }
