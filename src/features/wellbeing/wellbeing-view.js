    ...
    <section id="wellbeing-panel" class="panel stack">
      <div class="section-head">
        <div>
          ...
          <h3>Salud diaria con contexto</h3>
        </div>
        <p class="muted">Sintomas, ciclo y medicacion sin abrir un modulo pesado.</p>
      </div>

      <section class="wellbeing-summary compact-metrics">
        <article class="summary-card">
        ...
        <div class="stack">
          <article class="entry">
            <div>
              ...
              <p class="entry-note">${cycleSignals.length ? cycleSignals.join(" - ") : "Sin senales dominantes esta semana."}</p>
            </div>
          </article>
          <article class="entry">
            <div>
              <p class="entry-title">Digestion y energia</p>
              <p class="entry-meta">${weeklyHealth.digestionHeavyCount} registros digestivos pesados - energia media ${weeklyHealth.avgEnergy ? weeklyHealth.avgEnergy.toFixed(1) : "-"}/5</p>
        ...
        <section class="subpanel stack">
          ...
          <form id="symptom-form" class="stack">
            ...
            <div class="field-grid">
              ...
              <label><span>Digestion</span><select name="digestion"><option value="">Opcional</option>${digestionOptions}</select></label>
            </div>
            <div class="field-grid">
              <label><span>Energia (1-5)</span><select name="energy"><option value="">Opcional</option>${energyOptions}</select></label>
              <label><span>Animo (1-5)</span><select name="mood"><option value="">Opcional</option>${moodOptions}</select></label>
            </div>
      ...
      <section class="fold-grid">
        ${collapsiblePanel("Ultimos sintomas", "Ver registros", `<div class="stack">${renderSymptoms(state.cycle.symptomLog)}</div>`)}
        ${collapsiblePanel("Medicacion", "Lista actual", `<div class="stack">${renderMedications(state.medication)}</div>`)}
        ${collapsiblePanel("Biblioteca V1", "Sintomas base y familias operativas", `<section class="dashboard-summary">${renderSymptomGroups()}</section>`)}
      </section>
    </section>
  `;
}
