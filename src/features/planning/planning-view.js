...
function eventItems(events) {
  ...
  if (entries.length === 0) {
    return `<p class="muted">Aun no hay eventos guardados.</p>`;
  }
...
function blockItems(blocks) {
  if (blocks.length === 0) {
    return `<p class="muted">Aun no hay bloques de horario guardados.</p>`;
  }
      ...
      block => `
        <article class="entry">
          <div>
            ...
            <p class="entry-meta">${block.day} · ${block.start} - ${block.end}${block.category ? ` · ${block.category}` : ""}</p>
          </div>
...
function weeklyPrepItems(state) {
  ...
  if (nutritionReview.groupedShoppingList.length === 0 && nutritionReview.prepSuggestions.length === 0) {
    return `<p class="muted">Cuando haya planner semanal, aqui apareceran ingredientes a revisar antes del reset semanal.</p>`;
  }

  const shoppingItems = nutritionReview.groupedShoppingList.slice(0, 3).map(
    group => `<article class="entry"><div><p class="entry-title">${group.family}</p><p class="entry-meta">${group.items.slice(0, 3).map(item => `${item.name} (${item.count})`).join(" Â· ")}</p></div></article>`
  );
  ...
  return [...shoppingItems, ...prepItems]
    .join("");
}
...
function checklistItems(state) {
  ...
  if (items.length === 0) {
    return `<p class="muted">Aun no hay checklist semanal guardada para esta semana.</p>`;
  }
...
function gapFillItems(state, pack) {
  ...
  const sessionText = sessionSuggestions.length
    ? sessionSuggestions.slice(0, 3).map(item => `${item.date}: ${item.type} ${item.activity} (${formatSuggestionReason(item.reason)})`).join(" Â· ")
...
function nutritionPrepItems(pack) {
  ...
  if (!board || (board.batchItems.length === 0 && board.priorityMeals.length === 0)) {
    return `<p class="muted">Aun no hay suficiente planner para montar un pack nutricional semanal util.</p>`;
  }
...
function calibrationItems(calibration) {
  ...
  return `
    ...
    <article class="entry">
      <div>
        <p class="entry-title">Mapa de presion diaria</p>
  ...
  return `
    <section id="planning-panel" class="panel stack">
      <div class="section-head">
        <div>
          <p class="eyebrow">Planificacion migrada</p>
          <h3>Semana, calendario y horario</h3>
        </div>
        <p class="muted">La app empieza a unificar agenda, comida y entreno en una misma capa semanal.</p>
      </div>
      ...
      <section class="subpanel stack">
        <div class="section-head">
          <div>
            <p class="eyebrow">Planner asistido</p>
            <h4>Huecos que se pueden cerrar ya</h4>
          </div>
        </div>
      ...
      <section class="subpanel stack">
        <div class="section-head">
          <div>
            <p class="eyebrow">Pack nutricional</p>
            <h4>Compra y meal prep ya convertibles en sistema</h4>
          </div>
        </div>
      ...
      <section class="subpanel stack">
        <div class="section-head">
          <div>
            <p class="eyebrow">Recalibracion</p>
            <h4>Lo que la semana real te esta pidiendo</h4>
          </div>
        </div>
        <div class="button-row">
      ...
      ${collapsiblePanel("Semana real", "Proximas decisiones cruzadas", `<div class="timeline-list">${operationalTimelineItems(operationalTimeline)}</div>`)}
      ...
      <section class="subpanel stack">
        <div class="section-head">
          <div>
            <p class="eyebrow">Vista semana</p>
            <h4>Centro operativo semanal</h4>
          </div>
        </div>
        <section class="planning-summary">
          <article class="summary-card">
          ...
          <article class="summary-card">
            ...
            <p class="metric">${summary.events}</p>
            <p class="entry-meta">eventos en los proximos 7 dias</p>
          </article>
          <article class="summary-card">
            <p class="eyebrow">Reset semanal</p>
            <p class="metric">${resetDay}</p>
            <p class="entry-meta">dia actual de preparacion</p>
          </article>
          <article class="summary-card">
      ...
      <div class="planning-grid">
        <section class="subpanel stack">
          <div class="section-head">
            <div>
              <p class="eyebrow">Reset semanal</p>
              <h4>Checklist de preparacion</h4>
            </div>
          </div>
        ...
        <section class="subpanel stack">
          <div class="section-head">
            <div>
              <p class="eyebrow">Checklist semanal</p>
              <h4>Persistente</h4>
            </div>
          </div>
          <form id="weekly-form" class="stack">
            <div class="field-grid">
              ...
              <label><span>Dia de reset</span><select name="resetDay"><option ${resetDay === "Domingo" ? "selected" : ""}>Domingo</option><option ${resetDay === "Lunes" ? "selected" : ""}>Lunes</option><option ${resetDay === "Sabado" ? "selected" : ""}>Sabado</option></select></label>
            </div>
        ...
        <section class="subpanel stack">
          <div class="section-head">
            <div>
              <p class="eyebrow">Reset sugerido</p>
              <h4>Tareas automaticas</h4>
            </div>
          </div>
          ...
          <div class="stack">
            ...
            <article class="entry">
              <div>
                ...
                <p class="entry-meta">Cerrar adherencia semanal y luego conectar energia, sintomas y descanso.</p>
              </div>
            </article>
            <article class="entry">
              <div>
