...
export function getDailyCommandCenter(state, inputDate = new Date()) {
  ...
  if (sleepEntry && Number(sleepEntry.hours || 0) < 7) blockers.push("Recuperación corta");
  if (symptomsToday.some(item => Number(item.intensity || 0) >= 4)) blockers.push("Síntomas intensos");
  ...
  if (hydrationToday < hydrationGoal) focusAreas.push("subir hidratación");
  if (sleepEntry && Number(sleepEntry.hours || 0) < 7) focusAreas.push("bajar fricción");

  let focusHeadline = "Día estable para ejecutar lo previsto con poca fricción.";
  if (dayPressure?.status === "overloaded") {
    focusHeadline = "Día de carga alta: simplificar decisiones y proteger energía.";
  } else if (dayPressure?.status === "watch") {
    focusHeadline = "Día delicado: conviene ajustar el plan antes de forzarlo.";
  } else if (weeklyHealth.cycleContext.phase === "menstrual") {
    focusHeadline = "Fase menstrual: bajar carga y fricción.";
  }
...
export function getTodayDecisionBoard(state, inputDate = new Date()) {
  ...
  if (dailyCommand.hydrationToday < dailyCommand.hydrationGoal) {
    doNow.push("subir hidratación antes de que el día se cierre");
  }

  if (dailyCommand.dayPressure?.status === "overloaded") {
    lighten.push("simplificar decisiones y evitar meter más frentes hoy");
    supportBlocks.push({ kind: "recovery", title: "Recuperación", reason: "presión alta hoy" });
  } else if (dailyCommand.dayPressure?.status === "watch") {
    lighten.push("bajar ambición y proteger energía disponible");
    supportBlocks.push({ kind: "focus", title: "Ajuste de agenda", reason: "día en vigilancia" });
  }
  if (dailyCommand.weeklyHealth.cycleContext.phase === "menstrual") {
    lighten.push("dejar el día más suave por fase menstrual");
    supportBlocks.push({ kind: "recovery", title: "Recuperación", reason: "fase menstrual" });
  }
  ...
  if (dailyCommand.symptomsToday.length === 0) {
    captureNow.push("registrar cómo estás de energía, ánimo o digestión");
  }
  ...
  if (carryovers.length > 0) {
    captureNow.push("revisar arrastres recientes antes de seguir acumulando fricción");
  }
  ...
  const steps = [
      ...
      detail:
        nutritionReview.groupedShoppingList.length > 0
          ? nutritionReview.groupedShoppingList
              ...
              .join(" - ")
          : "Todavía no hay compra derivada del planner.",
    ...
    },
    {
      ...
      detail:
        ...
        "Aún no hay una base clara para batch cooking.",
    ...
    },
    {
      ...
      detail:
        plannedSessions.length > 0
          ...
          : "Aún no hay sesiones futuras cerradas.",
        ...
        dayLoad >= 2
          ...
          : isOverloaded
            ...
            : lowAdherenceWeek
              ...
              : isWatch
                ? "Sesión afinada para no tensar más la recuperación."
                : "Sesión sugerida para dar estructura mínima a la semana.",
      reason
    });
  });
...
export function getWeeklyPriorityBoard(state, inputDate = new Date()) {
  ...
  if (health.signals.some(signal => signal.kind === "training" || signal.kind === "training-energy")) {
    priorities.push({
      ...
      title: "Bajar fricción del entreno previsto",
      detail: "La semana muestra señales de baja adherencia o energía previa floja.",
      area: "entreno",
      severity: 2
    });
  }
...
export function getWeeklyCalibrationBoard(state, inputDate = new Date()) {
  ...
  const dayEntries = weekDates.map(date => {
    ...
    if (status === "overloaded") {
      recommendation = dayPlannedSessions.length > 0
        ? "Simplificar comida y bajar fricción del entreno previsto."
        : "Simplificar comidas y proteger recuperación.";
    } else if (status === "watch") {
      recommendation = dayEvents.length >= 2
        ? "Dejar cerrado lo esencial antes de que la agenda apriete."
        : "Vigilar energía y no improvisar de más.";
    }
  ...
  if (health.cycleContext.phase === "menstrual") {
    ...
    suggestedTasks.push("Bajar complejidad en fase lútea para no tensionar energía ni ánimo.");
  }

  let nextAdjustment = "Semana estable: mantener registro y pequeños ajustes.";
  if (topPressureDay?.status === "overloaded") {
    nextAdjustment = `El día más cargado es ${topPressureDay.date}; conviene simplificar comida, agenda o entreno.`;
  } else if (mealAdherence < 60 && totalPlannedMeals > 0) {
    nextAdjustment = "La comida planificada se está ejecutando poco; conviene recortar complejidad.";
...
export function getWeeklyReviewFlow(state, inputDate = new Date()) {
  ...
  const steps = [
    ...
    {
      key: "training",
      title: "Alinear entreno con energía real",
      status:
      ...
      action:
        preparationPack.suggestedSessions.length > 0
          ? "Completar entrenos sugeridos con la menor fricción posible."
          : "Revisar si la carga encaja con tu recuperación.",
      cta: preparationPack.suggestedSessions.length > 0 ? "apply-suggested-sessions" : ""
    },
    {
      key: "recovery",
      title: "Proteger descanso y energía",
      status:
      ...
      detail:
        health.avgSleepHours > 0
          ? `Sueño medio de ${health.avgSleepHours.toFixed(1)} h y ${health.signals.filter(signal => signal.kind === "recovery" || signal.kind === "fatigue").length} señales relacionadas.`
          : "Aún no hay suficiente sueño registrado para revisar.",
      action:
        health.avgSleepHours > 0 && health.avgSleepHours < 7
          ? "Bajar fricción de la semana y cuidar recuperación."
