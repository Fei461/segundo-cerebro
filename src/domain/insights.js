...
export function getWeeklyHealthInsights(state, inputDate = new Date()) {
  ...
  if (avgEnergy > 0 && avgEnergy <= 2.5) {
    signals.push({
      kind: "energy",
      title: "Energía baja en registros de síntomas",
      detail: `La energía media asociada a síntomas está en ${avgEnergy.toFixed(1)}/5.`
    });
  }
  ...
  if (avgMood > 0 && avgMood <= 2.5) {
    signals.push({
      kind: "mood",
      title: "Ánimo bajo en la semana",
      detail: `El ánimo medio registrado junto a síntomas está en ${avgMood.toFixed(1)}/5.`
    });
  }
  ...
  if (lowRecoverySessions.length >= 2) {
    signals.push({
      kind: "recovery",
      title: "Recuperación subjetiva floja",
      detail: `${lowRecoverySessions.length} sesiones con recuperación de 2/5 o menos.`
    });
  }
  ...
  if (lowEnergySessions.length >= 2) {
    signals.push({
      ...
      title: "Entrenos con energía previa baja",
      detail: `${lowEnergySessions.length} sesiones registradas con energía previa de 2/5 o menos.`
    });
  }
  ...
  if (mealSignals.length > 0) {
    ...
    signals.push({
      ...
      detail: `${top.name} aparece ${top.count} veces en comidas con reacción negativa registrada.`
    });
  }
  ...
  if (cycleContext.phase === "menstrual") {
    signals.push({
      ...
      detail: "Conviene priorizar margen, recuperación y entreno menos agresivo si la energía cae."
    });
  } else if (cycleContext.phase === "lutea" && (avgMood > 0 && avgMood <= 3 || avgEnergy > 0 && avgEnergy <= 3)) {
    signals.push({
      kind: "cycle-phase",
      title: "Fase lútea con energía o ánimo más bajos",
...
export function getCycleSupportSuggestions(state, inputDate = new Date()) {
  ...
  if (health.cycleContext.phase === "menstrual") {
    suggestions.push("Priorizar movilidad, paseo suave o recuperación si la energía cae.");
    suggestions.push("Simplificar comida y agenda para no tensionar más el día.");
  } else if (health.cycleContext.phase === "folicular") {
    suggestions.push("Buena ventana para empujar un poco más si la recuperación acompaña.");
    suggestions.push("Aprovechar foco y energía para cerrar preparación de la semana.");
  } else if (health.cycleContext.phase === "ovulatoria") {
    suggestions.push("Vigilar si sube la energía y usarla con intencion, no por impulso.");
    suggestions.push("Buen momento para sesiones clave si el sueño viene estable.");
  } else if (health.cycleContext.phase === "lutea") {
    suggestions.push("Bajar complejidad si notas más roce con energía, ánimo o digestión.");
    suggestions.push("Preparar comidas y agenda con más margen para evitar deuda operativa.");
  } else {
    suggestions.push("Seguir registrando días de ciclo para que la lectura futura gane precisión.");
  }
...
export function getWeeklyAutoSummary(state, inputDate = new Date()) {
  ...
  if (plannedSessions.length > 0 && adherenceTraining < 0.5) {
    reviewItems.push("Reducir fricción del entreno previsto para que la semana sea ejecutable.");
  }

  let headline = "Semana estable: mantener registro y pequeños ajustes.";
  if (watchouts.length >= 3) {
    headline = "Semana con varias señales activas: conviene revisar carga, comida y recuperación juntas.";
  } else if (health.cycleContext.phase === "menstrual") {
    headline = "Semana condicionada por fase menstrual: mejor margen y menos fricción.";
